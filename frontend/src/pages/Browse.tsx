    ...
    <div className="min-h-screen bg-[#080808] flex flex-col">
      ...
      <nav className="sticky top-0 z-40 bg-[#080808]/90 backdrop-blur border-b border-white/[0.05] px-6 py-3.5 flex items-center gap-4">
        ...
        <div className="flex-1 relative max-w-xl">
          ...
          <input
            className="w-full bg-zinc-900/80 border border-zinc-800 hover:border-zinc-700 rounded-xl pl-10 pr-10 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-[#FF6B1A]/50 focus:ring-1 focus:ring-[#FF6B1A]/20 transition-all"
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          {user && (
            <div className="flex items-center gap-2 pl-2 border-l border-white/[0.06] ml-1">
              <div className="w-7 h-7 rounded-full bg-[#FF6B1A]/20 flex items-center justify-center">
                <span className="text-xs font-bold text-[#FF6B1A]">{user.name[0]?.toUpperCase()}</span>
              </div>
          ...
          <p className="text-xs text-zinc-600">
            {loading ? "Loading…" : (
              <>
                ...
                {query && <> for "<span className="text-[#FF6B1A]">{query}</span>"</>}
              </>
            )}
          </p>
        </div>
